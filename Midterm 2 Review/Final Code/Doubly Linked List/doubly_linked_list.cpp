...
void add_node(node*& head_ptr, node*& tail_ptr, const char& payload)
{
	...
	else
	{
		...
		//if the data at head_ptr is greater than the payload
		if (head_ptr->data > payload)
		{
			//The added_node with data of payload is being placed before head_ptr
			added_node->next = head_ptr;
			added_node->prev = nullptr;
			head_ptr = added_node;
		}
		
		//if the data at head_ptr is less than the payload
		else
		{
			// adding a node after the head node
			node* cursor = head_ptr;
			//Cycle through head_ptr until payload is greater than the current value
...
bool remove_node(node*& head_ptr, node*& tail_ptr, const char& target)
{
	...
	{
		//if the head_ptr has nothing after it
		if (head_ptr->next == nullptr)
			tail_ptr = nullptr;
		//this deletes the first value of head_ptr
	...
	//cycle through removed_node_ptr as long as there is no match to the target
	...
	if (removed_node_ptr != nullptr && removed_node_ptr->next != nullptr)
	{
		...
		//removed_node_ptr->next has the value that matches the target
